...
class ANDEDashboard {
    ...
    updateSelectBehaviorForGlobalMode() {
        const shouldBeMultiple = this.globalMode === 'multiple';
        ['filterYear', 'filterTipoMedicion', 'filterTransformador'].forEach(id => {
            const sel = document.getElementById(id);
            if (!sel) return;
            if (sel.multiple !== shouldBeMultiple) {
                sel.multiple = shouldBeMultiple;
                if (!shouldBeMultiple) {
                    const selected = Array.from(sel.selectedOptions).map(o => o.value).filter(Boolean);
                    const keep = selected[0] || Array.from(sel.options).find(o => o.value)?.value || '';
                    sel.value = keep;
                }
            }
        });
    ...
    updateComparisonUiByGlobalMode() {
        this.updateSelectBehaviorForGlobalMode();
