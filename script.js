...
class ANDEDashboard {
    ...
    async loadData() {
        ...
        try {
            // Compatibilidad defensiva: mantenemos params para evitar ReferenceError en rutas legacy/caché
            ...
            params.set('anio', payload.anio || '2024');
            params.set('tipo_medicion', payload.tipo_medicion || 'TOTAL FEP');
            params.set('mes', payload.mes || 'all');
            if (payload.periodo) params.set('periodo', payload.periodo);
            console.log("🌐 POST /api/datos payload:", payload);
            ...
            try {
                ...
                if (isNetworkSuspended && seccionVal !== 'all') {
                    console.warn('⚠️ Reintentando carga con seccion=all por posible saturación de red');
                    payload.seccion = 'all';
                    res = await fetch(`${this.apiBaseUrl}/api/datos`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
    ...
    async refreshFepDepTotals() {
        try {
            ...
            console.log('🌐 Solicitando totales FEP/DEP (POST):', payload);
            const res = await fetch(`${this.apiBaseUrl}/api/datos`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
    ...
    updateRankingChart() {
        ...
        if (!Array.isArray(this.rankingChart.data.datasets) || !this.rankingChart.data.datasets.length) {
        ...
        const rankingDataset = this.rankingChart.data.datasets[0];
        ...
        console.log("🏆 Ranking mostrado (top10):", top);
        this.rankingChart.data.labels = top.map(t => t.label);
        rankingDataset.data = top.map(t => t.avg);
        rankingDataset.backgroundColor = top.map((_,i) => this.chartPalette[i%this.chartPalette.length] + '80');
        rankingDataset.borderColor = top.map((_,i) => this.chartPalette[i%this.chartPalette.length]);
